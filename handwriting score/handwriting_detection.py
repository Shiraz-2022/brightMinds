...
def preprocess_text(extracted_text):
    ...
    clean_text = clean_text.replace("1", "l")  
    clean_text = clean_text.replace("0", "O")  
    return clean_text
...
# Call this function (Final pipeline function)

# Returns the accuracy score and the final recognized text written by the user
def writing_detect(image_path, expected_text, option):
...
#     writing_detect(image_path, expected_text, option)