...
exports.submitQuestion = async (req, res) => {
  ...
  try {
    const response = await axios.post(
      `${FLASK_SERVER_URL}/submit-question`,
      form,
      {
        headers: {
          ...form.getHeaders(),
...
exports.getAudioAnswer = async (req, res) => {
  if (
    !req.files ||
    !req.files.audioFile ||
    req.files.audioFile.mimetype !== "audio/wav"
  ) {
    return res
      .status(400)
      .send("No audio file was uploaded or the file is not in WAV format.");
  }
  ...
  try {
    const googleResponse = await axios.post(
      `https://api.google.com/some-audio-processing-endpoint`,
      form,
      {
        headers: {
          ...form.getHeaders(),
    ...
    questions: [
      ...
      },
      {
        ...
        content: {
          ...
          correct_answer:
            "The cat and dog played in the sun. A bat flew nearby.",
          feedback: {
            correct: "Excellent job! You read the paragraph clearly.",
            incorrect:
              "Let's try reading this paragraph again. Take your time with each word.",
          },
        },
      },
      {
